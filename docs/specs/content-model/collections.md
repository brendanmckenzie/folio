...
- **Full-text search.** D1 supports FTS5 and it is the right answer for a search
  ...
  *2026-09-05:* it is spec 30, `full-text-search.md`, done the same day — it
  makes `search` a term of `ContentQuery` so this engine is the one that runs it.
...
Full-text search (**done**, `content-model/full-text-search.md`, landed 2026-09-05:
`search` is a term of `ContentQuery` and ranks through the same compiler); faceted
counts (one `group by`, unbuilt
