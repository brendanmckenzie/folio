...
*2026-09-05:* asked for, and built the same day. Spec 30
(`../content-model/full-text-search.md`) takes FTS5 for *published* content, and its
answer to the objection above is that there is no second write path: the FTS rows
come out of the same `indexStatements` in the same publish batch as `content_index`.
This route, the palette and both pickers stay on `like`, because they have to reach
drafts and the FTS index is written at publish.
...
- **FTS5 search.** Decision 8 — `like` plus `content_index` is enough for a
  picker, and an FTS table is a second write path. Built by spec 30, done
  2026-09-05, for published content only; see the note under decision 8.
