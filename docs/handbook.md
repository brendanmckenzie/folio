...
Every feature Folio has, what it is for, and why it is shaped the way it is.
This is the long document — read it when you want to understand something, not
when you want to start something.

- **Starting out?** [`README.md`](../README.md) is the tour, and its Quick
  start gets a site running.
- **Adding Folio to an existing project?** [`AGENTS.md`](../AGENTS.md) is the
  integration guide: what to do, in what order, and what you will get wrong.
- **Looking up a config key?** [`configuration.md`](configuration.md) is every
  knob on `createFolio()` and everything around it.
...
bin/folio.mjs    the CLI: `folio init`, `folio agents`
examples/starter a minimal consuming project — what `folio init` copies  (ships)
examples/demo/   a consuming project with everything switched on  (not shipped)
docs/            reference and specs  (the consumer-facing ones ship; specs do not)
...
`dist`, `src`, `migrations`, `bin`, `examples/starter`, and the consumer-facing
documents — `README.md`, `AGENTS.md`, `UPGRADING.md`, and `docs/`'s
`configuration.md`, `handbook.md`, `api.md` and `mcp.md`.
...
`migrations/` holds nine files: `0001_init.sql`, which is the whole base schema,
and `0002`–`0010` on top of it. (`0009` is a deliberate gap — it is a claim by a
spec still in draft.) The ten migrations that preceded `0001_init.sql` were
collapsed into it back when nothing was deployed; what each of them added is
still recorded, in its own spec's *Wire & schema changes* section and in the
table in `docs/specs/README.md`.

`0001_init.sql` is a plain `create table` file, so applying it over a database
that already has those tables **fails loudly** rather than adopting it. That is
deliberate: a silent adoption would leave an older schema in place. If your
local database predates the collapse, delete `.wrangler/state` and start from
`pnpm db:local`.

Upgrading a database that already has content — which migrations to apply and in
what order, and what else an upgrade needs — is [`UPGRADING.md`](../UPGRADING.md).
...
[`api.md`](api.md), and — the part that matters — **writing through the
...
is documented in [`mcp.md`](mcp.md)** — mint a token on the admin's Access
